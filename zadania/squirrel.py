...
LEVELUPTIME = 1.5    # how long the "Level Up" text stays on the screen in seconds
...
def runGame():
    ...
    lastLevelUpScore = 0      # score at which the last level up occurred
    currentGameSpeed = FPS    # current game speed (FPS)
    ...
    levelUpRect.center = (HALF_WINWIDTH, HALF_WINHEIGHT - 100)
    ...
    while True: # main game loop
            ...
            for i in range(len(squirrelObjs)-1, -1, -1):
                ...
                if 'rect' in sqObj and playerObj['rect'].colliderect(sqObj['rect']):
                    ...
                    if sqObj['width'] * sqObj['height'] <= playerObj['size']**2:
                        ...
                        # Check for level up (score is based on player size minus starting size)
                        currentScore = playerObj['size'] - STARTSIZE
                        if currentScore >= 5 and currentScore % 5 == 0 and currentScore > lastLevelUpScore:
                            # Level up!
                            levelUpMode = True
                            ...
                            currentGameSpeed = min(FPS * 1.5, FPS * (1 + (currentLevel - 1) * 0.1))
        ...
        # Check if level up mode should be turned off
        if levelUpMode and time.time() - levelUpStartTime > LEVELUPTIME:
            levelUpMode = False
            
        # Display level up message if in level up mode
        if levelUpMode:
            DISPLAYSURF.blit(levelUpSurf, levelUpRect)
            
            # Display current level
            levelInfoSurf = BASICFONT.render('Level %s' % (currentLevel), True, YELLOW)
            levelInfoRect = levelInfoSurf.get_rect()
            levelInfoRect.center = (HALF_WINWIDTH, HALF_WINHEIGHT - 60)
            DISPLAYSURF.blit(levelInfoSurf, levelInfoRect)
            
        # Display current level in the corner
        levelSurf = BASICFONT.render('Level: %s' % (currentLevel), True, WHITE)
        levelRect = levelSurf.get_rect()
        levelRect.topleft = (WINWIDTH - 150, 40)
        DISPLAYSURF.blit(levelSurf, levelRect)

        pygame.display.update()
        FPSCLOCK.tick(currentGameSpeed)
